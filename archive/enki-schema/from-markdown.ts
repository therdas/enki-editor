...
function getNodeType(schema: Schema, name: string): any {
  let found = schema.nodes[name];
  if(!found)  
    throw Error("Invalid node type in document");
  return found;
}
...
export const defaultMarkdownParser = new MarkdownParser(schema, MarkdownIt({html: false}), {
