...
export const LeafDirectiveMap = new Map<string, (arg: PMNode) => HTMLSpanElement>();

export function registerLeafDirective(name: string, functor: (arg: PMNode) => HTMLSpanElement) {
    LeafDirectiveMap.set(name, functor);
}
...
export class LeafDirectiveView implements NodeView {
    ...
    constructor(public node: PMNode, public outerView: EditorView, public getPos: () => number | undefined) {
        ...
        this.pluginDOM = LeafDirectiveMap.get(node.attrs.name)?.call(null, node) ?? this.createFallback(node);
